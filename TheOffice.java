...
public class TheOffice {
	...
	TheOffice(String fileLoc) throws TaskLevelException{
		...
		for(int i = 0; i < size; i++){
			...
			//Scanner read for Assistant Regional Manager
				...
				//set assistantManger to new assistant regional manager
			...
			//Scanner read for Sales Associate
				...
				//Get numclients and products
				numClients = Integer.parseInt(fs.nextLine().trim());
				//??????????????????????????????
				products = fs.nextLine().trim();
				...
				//set associate to new Sales Associate
				associate = new SalesAssociate(IDNumber, name, taskList, numClient, products, supervisor);
				
				//set workers[i] to associate
				workers[i] = associate;
			}
			//Scanner read for Receptionist
			else if(type.equals("Receptionist")){
				//Get maxTaskComplexityLevel
				...
				//set reception to new Receptionist
				reception = new Receptionist(IDNumber, name, taskList, supervisor, maxTaskComplexityLevel);
				
				//set workers[i] to reception
				workers[i] = reception;
	...
	public void setWorkers(Employee[] newWorkers){
		workers = newWorkers;
	}
	...
	public void levelDisplay(){
		...
		System.out.println("Name: " + super.getName() + "/tLevel: " + Task.getLevel())
	}
